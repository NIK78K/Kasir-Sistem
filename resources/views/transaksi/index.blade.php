    ...
    <div class="max-w-7xl mx-auto p-6">
        ...
        <div class="mb-8 flex gap-3">
            <a href="{{ route('barang.index') }}"
                class="px-5 py-2.5 bg-gray-600 text-white rounded-lg shadow-md hover:bg-gray-700 transition font-semibold flex items-center gap-2">
        ...
        <div class="mb-8 bg-white rounded-xl shadow-lg border border-gray-200 p-6">
            ...
            <form method="GET" action="{{ route('transaksi.index') }}"
                ...
                <select name="customer_id" id="customer_id"
                    class="flex-1 border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-600 transition bg-white"
                ...
                <a href="{{ route('customer.create') }}"
                    class="px-5 py-2.5 bg-gray-600 text-white rounded-lg shadow-md hover:bg-gray-700 transition font-semibold whitespace-nowrap flex items-center gap-2 justify-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <span
                            class="px-3 py-1 text-xs font-semibold rounded-full {{ ($customer->tipe_pembeli ?? '') == 'grosir' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700' }}">
        ...
        <div class="mb-8 bg-white rounded-xl shadow-lg border border-gray-200 p-6">
            ...
            <form method="GET" action="{{ route('transaksi.index') }}" class="flex gap-3">
                ...
                <input type="text" name="search_barang" placeholder="Masukkan nama barang..."
                ...
                <button type="submit"
                    class="px-6 py-2.5 bg-gray-600 text-white rounded-lg shadow-md hover:bg-gray-700 transition font-semibold whitespace-nowrap">
                    Cari Barang
                </button>
            </form>
        </div>
                    ...
                    @foreach ($barangs as $barang)
                        <div
                            ...
                            <div
                                ...
                                @if ($barang->isNew())
                                    <div
                                        class="absolute top-2 left-2 bg-green-500 text-white px-2 py-1 rounded-full text-xs font-bold shadow-lg">
                                        NEW
                                    </div>
                                @endif
                            </div>
                                    ...
                                    <form method="POST" action="{{ route('transaksi.addToCart') }}" class="space-y-3">
                                        ...
                                        <div class="flex flex-wrap gap-4 items-center">
                                            <div class="flex items-center gap-2">
                                                <label for="jumlah_{{ $barang->id }}"
                                                    class="text-sm font-medium">Jumlah:</label>
                                                <input type="number" id="jumlah_{{ $barang->id }}" name="jumlah"
                                                    value="1" min="1"
                                                    class="w-20 border border-gray-300 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-600" />
                                            </div>
                                        </div>
                                        <button type="submit"
                                            class="w-full px-4 py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-semibold flex items-center justify-center gap-2 add-to-cart-btn"
        ...
        {{-- Daftar Belanja --}}
        <div id="daftar-belanja" class="mb-8 bg-white rounded-xl shadow-lg border border-gray-200 p-6">
            <h2 class="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-indigo-600" fill="none"
                    viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            @if (session('cart') && count(session('cart')) > 0)
                    ...
                    <table class="w-full border-collapse">
                        <thead>
                            <tr class="bg-gradient-to-r from-gray-50 to-gray-100">
                                <th class="border border-gray-300 px-4 py-3 text-left font-semibold text-gray-800">Barang
                                </th>
                                <th class="border border-gray-300 px-4 py-3 text-left font-semibold text-gray-800">Tipe
                                    Harga</th>
                                <th class="border border-gray-300 px-4 py-3 text-center font-semibold text-gray-800">Jumlah
                                </th>
                                <th class="border border-gray-300 px-4 py-3 text-right font-semibold text-gray-800">
                                    Harga/Pcs</th>
                                <th class="border border-gray-300 px-4 py-3 text-right font-semibold text-gray-800">Total
                                    Harga</th>
                                <th class="border border-gray-300 px-4 py-3 text-center font-semibold text-gray-800">Aksi
                                </th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200">
                            ...
                            @foreach (session('cart') as $index => $item)
                                ...
                                <tr class="hover:bg-gray-50 transition">
                                    <td class="border border-gray-300 px-4 py-3 font-medium">{{ $item['nama_barang'] }}
                                    </td>
                                    <td class="border border-gray-300 px-4 py-3">
                                        <span
                                            class="px-2 py-1 text-xs font-semibold rounded-full {{ $item['tipe_harga'] == 'grosir' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700' }}">
                                            {{ ucfirst($item['tipe_harga']) }}
                                        </span>
                                    </td>
                                    <td class="border border-gray-300 px-4 py-3 text-center font-semibold">
                                        {{ $item['jumlah'] }}</td>
                                    <td class="border border-gray-300 px-4 py-3 text-right">Rp
                                        {{ number_format($item['harga'], 0, ',', '.') }}</td>
                                    <td class="border border-gray-300 px-4 py-3 text-right font-bold text-blue-600">Rp
                                        {{ number_format($itemTotal, 0, ',', '.') }}</td>
                                    <td class="border border-gray-300 px-4 py-3 text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <button
                                                class="decrease-qty bg-red-500 text-white w-8 h-8 flex items-center justify-center rounded text-sm hover:bg-red-600"
                                                data-index="{{ $index }}">
                                            ...
                                            <button
                                                class="increase-qty bg-green-500 text-white w-8 h-8 flex items-center justify-center rounded text-sm hover:bg-green-600"
                                                data-index="{{ $index }}">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none"
                                                    viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="mt-4 flex justify-between items-center">
                    ...
                    <button type="button"
                        class="bg-gradient-to-r from-green-600 to-green-700 text-white px-6 py-3 rounded-lg hover:from-green-700 hover:to-green-800 transition font-bold"
            ...
            @else
                <div class="text-center py-8">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-16 h-16 mx-auto text-gray-300 mb-3" fill="none"
                        viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                    </svg>
...
<x-modal name="payment-confirmation-modal" :show="false">
    <div class="p-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <button @click="$dispatch('close-modal', { name: 'payment-confirmation-modal' })"
